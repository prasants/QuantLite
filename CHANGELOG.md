...
### Lazy imports and a leaner core

- **Lazy package imports (PEP 562):** `quantlite/__init__.py` now imports
  submodules and public names on first access. Importing `quantlite.score` no
  longer pulls in pandas or matplotlib, so lightweight uses stay small (the
  score path needs only numpy and scipy), and `import quantlite` is faster. The
  full `from quantlite import X` and `quantlite.X` API is unchanged.
- **Annotation-only pandas imports** in `metrics`, `risk.metrics`, and
  `core.types` moved under `TYPE_CHECKING`, so the score path is pandas-free.
- Fixed `quantlite.__version__` (was stale at `1.6.0`).

---

## v1.7: The Firewall and Continuous Monitoring
